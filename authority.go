...
func (a *Authority) AssignRole(userID uint, roleName string) error {
	...
	// assign the role
	a.DB.Create(&UserRoleInt{UserID: userID, RoleID: role.ID})

	return nil
}
func (a *Authority) AssignRoleUUID(userID uuid.UUID, roleName string) error {
	// make sure the role exist
	var role Role
	res := a.DB.Where("name = ?", roleName).First(&role)
	if res.Error != nil {
	...
	// assign the role
	a.DB.Create(&UserRoleUUID{UserID: userID, RoleID: role.ID})
...
func (a *Authority) CheckRoleUUID(userID uuid.UUID, roleName string) (bool, error) {
	// find the role
	var role Role
	res := a.DB.Where("name = ?", roleName).First(&role)
	if res.Error != nil {
...
func (a *Authority) RevokeRole(userID uint, roleName string) error {
	...
	// revoke the role
	a.DB.Where("user_id = ?", userID).Where("role_id = ?", role.ID).Delete(UserRoleInt{})

	return nil
}

func (a *Authority) RevokeRoleUUID(userID uuid.UUID, roleName string) error {
	// find the role
	var role Role
	res := a.DB.Where("name = ?", roleName).First(&role)
	if res.Error != nil {
	...
	// revoke the role
	a.DB.Where("user_id = ?", userID).Where("role_id = ?", role.ID).Delete(UserRoleUUID{})
...
func (a *Authority) GetUserRolesUUID(userID uuid.UUID) ([]string, error) {
	var result []string
	var userRoles []UserRoleUUID
	a.DB.Where("user_id = ?", userID).Find(&userRoles)
